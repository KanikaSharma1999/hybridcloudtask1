...
resource "aws_instance" "web1" {
 ...
 connection {
    type     = "ssh"
    user     = "ec2-user"
    private_key = file("C:/Users/DeLL/Downloads/kanikakey1.pem")
...
resource "null_resource" "image" {
  provisioner "local-exec" {
    command = "git clone https://github.com/KanikaSharma1999/hybridcloudtask1.git images"
  }
}
...
resource "aws_s3_bucket_object" "object" {
  bucket = aws_s3_bucket.kanika.bucket
  acl = "public-read"
  key    = "terraform.png"
  source = "images/terraform.png"
depends_on=[aws_s3_bucket.kanika,null_resource.image]
...
resource "aws_cloudfront_distribution" "my_distribution" {
    origin {
         domain_name = "${aws_s3_bucket.kanika.bucket_regional_domain_name}"
         origin_id   = "${aws_s3_bucket.kanika.id}"
...
default_cache_behavior {
    ...
    target_origin_id = "${aws_s3_bucket.kanika.id}"
...
restrictions {
 ...
 depends_on=[aws_s3_bucket.kanika]
}
...
provisioner "remote-exec" {
    inline = [
      ...
      "sudo git clone https://github.com/KanikaSharma1999/hybridcloudtask1.git /var/www/html/",
      "sudo su << EOF",
